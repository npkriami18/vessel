#!/usr/bin/env python3
"""
learning16: Team Protocols — request-response protocol + request_id + dispatch + state machine.

Run:  python learning16_team_protocols/code.py
Need: pip install openai python-dotenv + .env with AZURE_ENDPOINT, AZURE_API_KEY, AZURE_API_VERSION, AZURE_DEPLOYMENT
...
class _ToolCallAdapter:
    """Adapts an Azure tool_call to the internal interface (name/input/id)."""
...
def agent_loop(messages: list, context: dict):
    ...
    while True:
        ...
        # Background notifications go in a separate user message (Azure rule: no mixed role)
...
if __name__ == "__main__":
    print("learning16: team protocols")
    print("Enter a question, press Enter to send. Type q to quit.\n")
    history = []
    context = update_context({}, [])
    while True:
        try:
            query = input("\033[36mlearning16 >> \033[0m")
