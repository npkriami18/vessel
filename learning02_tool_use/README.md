# learning02: Tool Use — Add a Tool, Add Just One Line

learning01 → `learning02` → [learning03](../learning03_permission/) → learning04 → ... → s20
...
The learning01 Agent has only one tool: bash. To read a file, `cat`; to write, `echo "..." > file.py`; to edit, `sed`.
...
The learning01 loop is fully preserved (LLM call, stop_reason check, message append — not a single word changed). The only change is in that one line of tool execution: `run_bash()` is replaced with `TOOL_HANDLERS[block.name]()` dispatch lookup.
...
learning02 expands to 5 tools, each independently defined:
...
| Loop Unchanged | learning01's `while True` loop — not a single line changed |

---

## Changes from learning01

| Component | Before (learning01) | After (learning02) |
|-----------|-------------|-------------|
...
| Loop | `while True` + `stop_reason` | Identical to learning01 |
...
python learning02_tool_use/code.py
```
...
→ learning03 Permission: Add a gate before tool execution — is this operation safe? Does it need user approval?
...
3. **PreToolUse hooks** (`800-862`, covered in learning04): hooks can return messages, modify input, or block execution
4. **Permission check** (`921-931`, core topic of learning03): canUseTool + checkPermissions → allow/deny/ask
