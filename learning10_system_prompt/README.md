# learning10: System Prompt — Assembled at Runtime, Never Hardcoded

learning01 → ... → learning08 → learning09 → `learning10` → [learning11](../learning11_error_recovery/) → learning12 → ... → learning20
...
From learning01 to learning09, the system prompt was always one hardcoded line:

```python
SYSTEM = f"You are a coding agent at {WORKDIR}. Use tools to solve tasks."
```

That worked for learning01 — only bash, read, write. But by learning09, the agent has memory, compression, skill loading. The prompt needs to describe more and more capabilities:
...
learning10 focuses on prompt assembly. It builds on the learning08-learning09 capabilities but doesn't re-implement compression or memory. The core change: split the hardcoded `SYSTEM` into independent sections, assemble them at runtime based on real state, and cache the result.
...
| Component | Before (learning09) | After (learning10) |
|-----------|-------------|-------------|
...
python learning10_system_prompt/code.py
```
...
learning11 Error Recovery → four recovery paths. Upgrade tokens, compress context, exponential backoff, switch models.
