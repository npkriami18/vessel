#!/usr/bin/env python3
"""
learning20: Comprehensive Agent — all teaching components in one loop.

Run:  python learning20_comprehensive/code.py
Need: pip install openai python-dotenv pyyaml + .env with AZURE_* vars
...
PRIMARY_MODEL = MODEL
FALLBACK_MODEL = os.getenv("AZURE_FALLBACK_DEPLOYMENT")
...
PROMPT = "\033[36mlearning20 >> \033[0m"
CLI_ACTIVE = False
    ...
    def run():
        ...
        while True:
            ...
            for _ in range(10):
                ...
                try:
                    # Safe tail slice using _group_messages to never split atomic pairs
                    tail = [m for g in _group_messages(messages)[-10:] for m in g]
                ...
                for tc in (assistant_msg.tool_calls or []):
                    ...
                    if fn_name == "submit_plan":
                        ...
                        output = call_tool_handler(handler, args, fn_name)
                    messages.append({
                        "role": "tool",
                        "tool_call_id": tc.id,
                        "content": str(output),
        ...
        # Extract last assistant text from plain dict messages
        summary = "Done."
...
def permission_hook(block):
    # block is a dict (Azure tool_call) or object; normalise access
    ...
    if name.startswith("mcp__") and "deploy" in name:
        print(f"\n\033[33m[permission] MCP destructive-looking tool: {name}\033[0m")
        choice = input("  Allow? [y/N] ").strip().lower()
        if choice not in ("y", "yes"):
            return "Permission denied by user"
    return None
...
def log_hook(block):
    name = block.get("name") if isinstance(block, dict) else getattr(block, "name", "")
    print(f"\033[90m[HOOK] {name}\033[0m")
    return None
...
def large_output_hook(block, output):
    ...
    if len(str(output)) > 100000:
        print(f"\033[33m[HOOK] large output from {name}: "
...
def extract_text_from_messages(messages: list) -> str:
    """Extract the last assistant text response from an Azure-format message list."""
    for msg in reversed(messages):
        if msg.get("role") == "assistant" and msg.get("content"):
...
def has_tool_calls(choice) -> bool:
    """True if the Azure choice has pending tool calls."""
...
def spawn_subagent(description: str) -> str:
    ...
    for _ in range(30):
        ...
        if not has_tool_calls(choice):
            break

        for tc in (assistant_msg.tool_calls or []):
            fn_name = tc.function.name
            args = json.loads(tc.function.arguments)
            # Build a minimal dict so permission hooks can inspect it uniformly
            fake_block = {"name": fn_name, "input": args}
    ...
    return extract_text_from_messages(messages) or "Subagent finished without a text summary."
...
def message_has_tool_calls(message: dict) -> bool:
...
def collect_tool_results(messages: list):
    return [(i, msg) for i, msg in enumerate(messages)
            if msg.get("role") == "tool"]
...
def tool_result_budget(messages: list, max_bytes: int = 200_000) -> list:
    ...
    if total <= max_bytes:
        return messages
    for _, msg in sorted(tool_msgs,
                         key=lambda pair: len(str(pair[1].get("content", ""))),
                         reverse=True):
        if total <= max_bytes:
            break
        text = str(msg.get("content", ""))
        msg["content"] = persist_large_output(
            msg.get("tool_call_id", "unknown"), text)
        total = sum(len(str(m.get("content", ""))) for _, m in tool_msgs)
    return messages


def snip_compact(messages: list, max_messages: int = 50) -> list:
    """Snip old messages, always keeping atomic assistant+tool pairs intact."""
    if len(messages) <= max_messages:
        return messages
    groups = _group_messages(messages)
    if len(groups) <= max_messages:
        return messages
    # Keep first group (usually initial user message) + recent groups
    head = groups[:1]
    tail_groups = groups[-(max_messages - 1):]
    snipped = len(groups) - len(head) - len(tail_groups)
    flat_head = [m for g in head for m in g]
    flat_tail = [m for g in tail_groups for m in g]
    return (flat_head
            + [{"role": "user", "content": f"[snipped {snipped} message group(s)]"}]
            + flat_tail)
...
def reactive_compact(messages: list) -> list:
    ...
    # Safe tail using _group_messages to avoid starting mid-pair
    tail = [m for g in _group_messages(messages)[-5:] for m in g]
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if ("overloaded" in name or "529" in msg or "overloaded" in msg
                    or "503" in msg or "service unavailable" in msg):
                state.consecutive_529 += 1
                if state.consecutive_529 >= MAX_CONSECUTIVE_529 and FALLBACK_MODEL:
                    state.current_model = FALLBACK_MODEL
                    state.consecutive_529 = 0
                    print(f"  \033[31m[503/529] switching to {FALLBACK_MODEL}\033[0m")
                delay = retry_delay(attempt)
                print(f"  \033[33m[503] retry {attempt + 1}/{MAX_RETRIES} "
                      f"after {delay:.1f}s\033[0m")
                time.sleep(delay)
                continue
            raise
...
def is_prompt_too_long_error(e: Exception) -> bool:
    msg = str(e).lower()
    return (("context_length_exceeded" in msg)
...
def start_background_task(fn_name: str, args: dict, tc_id: str,
                          handlers: dict) -> str:
    global _bg_counter
    _bg_counter += 1
    bg_id = f"bg_{_bg_counter:04d}"
    command = args.get("command", fn_name)

    def worker():
        handler = handlers.get(fn_name)
        result = call_tool_handler(handler, args, fn_name)
        fake_block = {"name": fn_name, "input": args}
        trigger_hooks("PostToolUse", fake_block, result)
...
def assemble_tool_pool() -> tuple[list[dict], dict]:
    """Merge builtin tools + all MCP tools into one pool (Azure format)."""
...
BUILTIN_TOOLS = [
    ...
    _fn("load_skill", "Load the full content of a skill by name.",
        {"type": "object",
    ...
    _fn("list_crons", "List registered cron jobs.",
        {"type": "object", "properties": {}, "required": []}),
    _fn("cancel_cron", "Cancel a cron job by ID.",
        {"type": "object",
...
def build_user_notifications() -> list[dict]:
    """Collect completed background task notifications as user text blocks."""
    notes = collect_background_results()
    return [{"role": "user", "content": note} for note in notes]


def inject_background_notifications(messages: list):
    for note_msg in build_user_notifications():
...
def agent_loop(messages: list, context: dict):
    ...
    while True:
        ...
        # finish_reason "length" is the Azure equivalent of stop_reason "max_tokens"
        if choice.finish_reason == "length":
            ...
            if not state.has_escalated:
                ...
                print(f"  \033[33m[length] retry with {max_tokens}\033[0m")
                continue
        ...
        for tc in (assistant_msg.tool_calls or []):
            ...
            if should_run_background(fn_name, args):
                bg_id = start_background_task(fn_name, args, tc.id, handlers)
                messages.append({
                    "role": "tool",
                    "tool_call_id": tc.id,
        ...
        if compacted_now:
            continue
        # Loop continues; the tool results are already appended individually
...
if __name__ == "__main__":
    CLI_ACTIVE = True
    print("learning20: comprehensive agent")
