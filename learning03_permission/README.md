# learning03: Permission — Check Permissions Before Execution

learning01 → learning02 → `learning03` → [learning04](../learning04_hooks/) → learning05 → ... → learning20
...
learning02's Agent has 5 tools. File tools are protected by `safe_path`, but bash is unrestricted. Ask it to "clean up the project," and it might run `rm -rf /`.
...
learning02's loop is fully preserved. The only change is inserting `check_permission()` before tool execution — each tool call passes through three gates in a fixed order: hard deny first, then soft ask, and if neither matches, allow.
...
# In agent_loop — learning02's loop with just one line added:
for block in response.content:
    if block.type == "tool_use":
        if not check_permission(block):           # ← NEW
            results.append({... "content": "Permission denied."})
            continue
        output = TOOL_HANDLERS[block.name](**block.input)  # learning02 original
        results.append(...)
```

---

## Changes from learning02

| Component | Before (learning02) | After (learning03) |
|-----------|-------------|-------------|
...
→ learning04 Hooks: Add hooks to the loop. Extension logic hangs on hooks; the loop stays clean.
