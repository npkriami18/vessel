#!/usr/bin/env python3
"""
learning15: Agent Teams — MessageBus + spawn_teammate_thread + inbox injection.

Run:  python learning15_agent_teams/code.py
Need: pip install openai python-dotenv + .env with AZURE_API_KEY, AZURE_ENDPOINT, AZURE_API_VERSION, AZURE_DEPLOYMENT
...
# ── Task System (from learning12, synced) ──
...
# ── Prompt Assembly (from learning10, synced) ──
...
# ── Background Tasks (from learning13, synced) ──
...
class _TC:
    """Lightweight adapter so execute_tool/start_background_task work with Azure tool calls."""
    def __init__(self, name: str, input_dict: dict, tc_id: str):
        self.name = name
        self.input = input_dict
        self.id = tc_id


def execute_tool(block: _TC) -> str:
...
def start_background_task(block: _TC) -> str:
...
# ── Cron Scheduler (from learning14, synced) ──
...
# ── Teammate Thread (learning15 new) ──
    ...
    def run():
        ...
        for _ in range(10):
            ...
            for tc in (assistant_msg.tool_calls or []):
                ...
                output = handler(**tool_input) if handler else "Unknown"
...
# ── Team Tool Handlers (learning15 new) ──
...
def agent_loop(messages: list, context: dict):
    ...
    while True:
        ...
        for tc in (assistant_msg.tool_calls or []):
            ...
            block = _TC(tool_name, tool_input, tc.id)
        ...
        # Background notifications go in a separate user message (Azure forbids mixed role=tool/text)
...
if __name__ == "__main__":
    print("learning15: agent teams")
    print("Enter a question, press Enter to send. Type q to quit.\n")
    history = []
    context = update_context({}, [])
    while True:
        try:
            query = input("\033[36mlearning15 >> \033[0m")
